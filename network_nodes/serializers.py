...
class NetworkNodeSerializer(serializers.ModelSerializer):
    ...
    name = serializers.CharField(
        required=True, error_messages={"required": "Название звена сети обязательно."}
    )
    ...
    phone = PhoneNumberField(
        allow_null=True, required=False, help_text="Укажите телефон"
    )
    country = serializers.CharField(
        required=True, error_messages={"required": "Страна обязательна для заполнения."}
    )
    city = serializers.CharField(
        required=True, error_messages={"required": "Город обязателен для заполнения."}
    )
    street = serializers.CharField(
        allow_null=True, required=False, help_text="Укажите улицу"
    )
    ...
    supplier_name = serializers.CharField(
        source="supplier.name", read_only=True, help_text="Название поставщика"
    )
